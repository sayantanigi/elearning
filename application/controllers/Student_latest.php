              ...
              if(in_array($index_day,$allowedDayArr)){
					        ...
					        $list[$index]['allDay'] = true;
							    ...
							    if (date('m', $time)==$current_month && in_array($index_day,$allowedDayArr) 
							    	  || in_array($current_full_date,$currentBookedClassArr)){ 

							    	$all_bkd_position = array_search($current_full_date, $bookedClassArr);  

							      print_r($bookedClassArr);
						          ...
						          $list[$index]['courseLvl'] = ucfirst($bookedCourseArr[$all_bkd_position]['level']);
											...
											if(!empty($currentBookedClassArr) && in_array($current_full_date,$currentBookedClassArr)){
												...
												$list[$index]['date']=$current_full_date;
				                     ...
				                     $list[$index]['color'] = "#fd6a03";//$scheduleBgColorCdArr[0];
						        	   ...
						        	   //$list[$index]['classRawDate']=date('Y-m-d', $time);
											   ...
											   //echo $current_full_date;exit;
											
						        	   $fromTime = $classTimeArr['fromTime'];
										     $toTime = $classTimeArr['toTime'];

												 $list[$index]['classDate']=date('jS F, Y',strtotime($current_full_date));
						        	   $list[$index]['classRawDate']=date('Y-m-d',strtotime($current_full_date));
				                    ...
				                    $list[$index]['color'] = "#db5382";
								     ...
								     $insSetFrmTime = new DateTime($insSetFrmTime, new DateTimeZone($instructorDetail->timezone));
										 ...
										 $insSetFrmTime->setTimezone(new DateTimeZone($stu_time_zone));
									   $finalInsSetFrmTime = $insSetFrmTime->format('Y-m-d H:i:s');
									   $checkBookedColidedDate = $insSetFrmTime->format('Y-m-d');
										 ...
										 //echo $finalInsSetFrmTime."<br>".$finalInsSetToTime."<br>";

										 //if(!in_array($checkBookedColidedDate,$bookedClassArr)){
                         //echo $finalInsSetFrmTime."<br>";
										 //}
	...
	public function manageStudentSessionData(){
     ...
     if($purchaseCount>0){
      ...
      //var_dump($classBooked);exit; 
		  ...
		  if(in_array($current_day,$allowedDayArr) && $classBooked == false){ 
