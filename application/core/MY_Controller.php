...
class MY_Controller extends CI_Controller 
{
	...
	{
		if (!$this->session->has_userdata('userId') || !$this->session->has_userdata('student'))
		{
			$redirectto = urlencode(current_url());
			$loginmsg = '["Please login to access this features!", "error", "#DD6B55"]';
			$this->session->set_flashdata('msg', $loginmsg);		
			redirect(base_url(),'refresh');
		}
	}

	public function instructorLoggedIn()
	{
		$this->session->userdata('userId');
		$this->session->userdata('userType'); 

		if (!$this->session->has_userdata('userId') || !$this->session->has_userdata('instructor'))
		{
			$redirectto = urlencode(current_url());
			$loginmsg = '["Please login to access this features!", "error", "#DD6B55"]';
			$this->session->set_flashdata('msg', $loginmsg);		
	...
	public function mail($data)
	{
		...
		$this->email->from('no_reply@truecostid.com', 'True Cost Id');
		$this->email->to($data['email']);
		$this->email->subject($data['subject'].' | '.'True Cost Id');
