		...
		<div class="container-fluid">
			...
			<div class="d-flex flex-wrap mb-4 row">
				...
				<div class="col-xl-6 col-lg-6 d-flex flex-wrap">
					<div class="ml-auto">
						<a href="<?= admin_url('reports/changeInsHistory') ?>" class="btn btn-rounded btn-info">
							<span> <i class="fa fa-history color-info"></i></span> View Change Instructor History 
						</a>				
					</div>
				</div>
									...
									<tbody>
										...
										<?php 
											foreach ($changeInstructorData as $key => $v): 
												...
												$sql_fetch_booked_class = "SELECT TIMEDIFF(sbc.toTime , sbc.fromTime) as timeDiff FROM student_booked_classes sbc WHERE sbc.courseId='$courseId' AND sbc.courseLvl='$courseLvl' AND sbc.studentId='$studentId' AND sbc.instructorId='$instructorId' AND sbc.classDate < CURDATE()";
										...
										?>
											<tr class="odd" role="row">
												...
												<td class="text-center actionbtnlist">
													<a href="javascript:void(0);" class="btn btn-xs btn-primary" data-toggle="tooltip2" title="View Details" onclick="showReason(<?=$v->queryId?>)">
														<i class="fa fa-eye"></i>
													</a>

													<a href="javascript:void(0)" onclick="approveQuery(<?= @$v->queryId ?>)" class="btn btn-xs btn-success" data-toggle="tooltip2" title="Approve Query">
														<i class="fa fa-check"></i>
													</a>
													
													<a href="javascript:void(0)" onclick="denyQuery(<?= @$v->queryId ?>)" class="btn btn-xs btn-danger" data-toggle="tooltip2" title="Deny query">
	...
	<script>
		function approveQuery(queryId) {
			swal({
				...
				if (isConfirm) {
					window.location.href = '<?= admin_url('reports/approveQuery/') ?>'+queryId
				}
			});
		}

		function denyQuery(queryId) {
			swal({
				title: 'Are You sure want to deny this Request?',
				type: 'warning',
				showCancelButton: true,
				confirmButtonColor: '#A5DC86',
				cancelButtonColor: '#DD6B55',
				...
				if (isConfirm) {
					window.location.href = '<?= admin_url('reports/deleteQuery/') ?>'+queryId
				}
			});
		}
