...
?>
										...
										<tr class="odd" role="row">
									       <input type="hidden" id="cancelReason" value="<?=strip_tags(html_entity_decode($cancelCourseDetails->descriptions))?>">
											...
											<td class="text-center actionbtnlist">
												...
												<a href="javascript:void(0)" onclick="approveCourseCancellation(<?= @$cancelCourseDetails->requestId ?>)" class="btn btn-xs btn-success" data-toggle="tooltip2" title="Approve Cancellation">
													<i class="fa fa-check"></i>
												</a>

												<a href="javascript:void(0)" onclick="denyCourseCancellation(<?= @$cancelCourseDetails->requestId ?>)" class="btn btn-xs btn-danger" data-toggle="tooltip2" title="Deny Cancellation">
	...
	<script>
		...
		function approveCourseCancellation(requestId) {
			/*swal({
			...
			swal({
				...
				if (isConfirm) {
					window.location.href = '<?= admin_url('reports/approveCourseCancellation/') ?>'+requestId
				}
			});
		}

		function denyCourseCancellation(requestId) {
			swal({
				...
				if (isConfirm) {
					window.location.href = '<?= admin_url('reports/denyCourseCancellation/') ?>'+requestId
				}
			});
		}
